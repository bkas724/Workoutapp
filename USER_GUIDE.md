...
* **Up-Next Action Card**: Displays the immediate workout queued by the JIT engine, highlighting target pace, exact duration, workout type, movement prep tips, a dynamic **Prescription Subtitle** (e.g. `5x 800m • Target ~7:15/mi`, `4.5 Miles • Target ~8:15/mi`, or `Circuit • 3 Rounds`), and pre-run notes.
...
* **Ephemeral Weight Check-in (ADHD Guilt-Free Prompt)**: When weight hasn't been logged in over 2 days, a gentle check-in prompt appears on launch with a polite 9-second window. If untouched, it gracefully dissolves into the background without lingering visual debt or guilt, while remaining available in the **Analytics** tab anytime on demand.
* **Daily Fuel & Health Insights Card**: Provides seamless, edge-to-edge daily recovery guidance—including movement preparation cues, hydration targets, and effort-matched nutrition strategies (Rest, Light, or Hard day meals) rendered with clean, high-contrast color badges and borderless layouts that eliminate visual friction and decision fatigue.
