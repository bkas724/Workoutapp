...
*   **Time-Based Interval Workouts ($N \times T\text{ mins}$):**
    $$\text{Miles Per Rep} = \frac{\text{Rep Duration (minutes)}}{\text{Pace Decimal (min/mile)}} = \frac{5.0}{7 + 8/60} \approx 0.701\text{ mi}$$
    $$\text{Total Work Mileage} = N \times \text{Miles Per Rep} \approx 3 \times 0.701 = 2.10\text{ mi}$$
*   **Distance-Based Interval Workouts ($N \times D\text{ meters/km/miles}$):**
    $$\text{Total Work Mileage} = N \times (D \times \text{conversion factor})$$
*   **Chart Aggregation Integration:** By persisting `actualLoggedDistance`, weekly volume bar charts and macrocycle mileage summaries accurately account for all interval and tempo sessions without manual distance conversions.
